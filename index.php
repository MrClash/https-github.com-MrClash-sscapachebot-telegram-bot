<?php
ob_start();
define('API_KEY','224375040:AAFR6cKVwdTAHgBZRbtZ60_Vi_fKxbopT0w');
                ...
                'results' => json_encode([[
                    ...
                    [
                        ...
                        'input_message_content'=>[
                            'message_text'=>'@inlinerrobot getlist-'.$theListId
                        ]
                    ]])
            ]));
        }else{
...
<pre>@inlinerrobot d6cdbea45b238632bdd6d11dcf7fe98f </pre>
...
قبل از هر کاری اول باید بدانید این via inlinerrobot چیست !
...
<code>@inlinerrobot getlist-XXXXX </code>
...
👈  حالا برای من کد inline تان را بفرستید : ( فعلا فقط لیست ها قبول هستند )".$cancel_Text,
                ...
                case '2':{
                    ...
                    if(str_replace('@inlinerrobot getlist-','',$code) != $code){
                        $code_list = trim(str_replace('@inlinerrobot getlist-','',$code));
...
به ربات ما سر بزنید
https://telegram.me/inlinerrobot
                            "
                        ]);
                    }else{
                        $newList = [];
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                            ...
                            $theCommand = '@inlinerrobot getlist-'.ListTab::getCurrentMakeID($update->message->from->id);
                            makeHTTPRequest('sendMessage',[
                                'chat_id'=>$update->message->from->id,
                                'text'=>$theCommand,
                                'reply_markup'=>json_encode([
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                    ...
                    $theCommand = '@inlinerrobot getbanner-'.getCurrentMakeID($update->message->from->id);
                    makeHTTPRequest('sendMessage',[
                        'chat_id'=>$update->message->from->id,
                        'text'=>$theCommand,
                        'reply_markup'=>json_encode([
...
//                }break;
                ...
                case '10':{
                    ...
                    makeHTTPRequest('sendMessage',[
                        'text'=>'@inlinerrobot getbanner-'.getCurrentMakeID($userID),
                        'chat_id'=>$userID
                    ]);
                }break;
